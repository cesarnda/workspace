...
public class RandomRepeatedTestCases {

	private static final Logger logger = LoggerFactory.getLogger(RandomRepeatedTestCases.class);
	...
	public void getAllResidue() throws FileNotFoundException{
		RandomRepeated randomRepeated = new RandomRepeated();
		...
		for(int index = 1; index <= NUMBER_OF_PROBLEM_INSTANCES; index++){
			...
			long residue = randomRepeated.applyRandomRepeated(FILE_NAME_PREFIX + (index < 10 ? "0" : "") + index + ".txt");
			long timeTaken = System.currentTimeMillis() - startTime;
			solutions.add(new NumberPartitionSolution(timeTaken, FILE_NAME_PREFIX + (index < 10 ? "0" : "") + index + ".txt", residue, "RandomRepeated"));
		}
