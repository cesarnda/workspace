...
public class MixedRandomRepeatedTestCases {

	private static final Logger logger = LoggerFactory.getLogger(MixedRandomRepeatedTestCases.class);
	...
	public void getAllResidue() throws FileNotFoundException{
		MixedRandomRepeated mixedRandomRepeated = new MixedRandomRepeated();
		...
		for(int index = 1; index <= NUMBER_OF_PROBLEM_INSTANCES; index++){
			...
			long residue = mixedRandomRepeated.getResidue(FILE_NAME_PREFIX + (index < 10 ? "0" : "") + index + ".txt");
			long timeTaken = System.currentTimeMillis() - startTime;
			solutions.add(new NumberPartitionSolution(timeTaken, FILE_NAME_PREFIX + (index < 10 ? "0" : "") + index + ".txt", residue, "MixedRandomRepeated"));
		}
